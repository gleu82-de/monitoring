...
if [[ "$CURRENT_HOST" != "$TARGET_HOST" ]]; then
    echo "INFO: Script läuft auf $CURRENT_HOST, führe Checks via SSH auf PROD aus..."
    
    # Über SSH auf PROD verbinden und Commands ausführen
    ssh dgl@PROD << 'EOF'
        # KUMA_URL extrahieren
        CONFIG="/home/dgl/Projekte/monitoring/config/kuma.conf"
        KUMA_URL=$(grep '^KUMA_URL=' "$CONFIG" | cut -d= -f2 | tr -d '"')
        echo "KUMA_URL: $KUMA_URL"

        # Services aus der Config verarbeiten
...
EOF
    ...
    echo "INFO: SSH-Verbindung zu PROD beendet"
    exit 0
fi

# Wenn wir bereits auf PROD sind, lokal ausführen
echo "INFO: Script läuft bereits auf $TARGET_HOST"
